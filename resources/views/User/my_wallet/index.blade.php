                                    ...
                                    <div class="table-responsive">
                                        <table id="table-breakpoint">
                                           ...
                                           @foreach($wallets as $wallet)
                                            <tr>
                                                <td>{!! clean($wallet->name) !!}</td>
                                                <td>{!! clean($wallet->balance) !!}</td>
                                                <td>{!! clean($wallet->referral_balance) !!}</td>
                                                <td class="updated_at"> <p>{{date('Y-m-d',strtotime($wallet->updated_at))}}   {{date('H:i:s',strtotime($wallet->updated_at))}}</p> </td>
                                                <td>
                                                    <div class="action-icon d-flex">
                                                        <a title="{{__('Deposit')}}" href="{{route('walletDetails',$wallet->id)}}?q=deposit" class="wallet"><i class="flaticon-wallet"></i></a>
                                                        <a title="{{__('withdraw')}}" href="{{route('walletDetails',$wallet->id)}}?q=withdraw" class="send"><i class="flaticon-send"></i></a>
                                                        <a title="{{__('Activity')}}" href="{{route('walletDetails',$wallet->id)}}?q=activity" class="share"><i class="flaticon-share"></i></a>
                                                        @if($wallet->is_primary == 0)
                                                            <a title="{{__('Make primary')}}" href="{{route('makeDefaultAccount',$wallet->id)}}" class="send"><i class="fa fa-key"></i></a>
                                                        @endif
                                                    </div>
                                                </td>
                                            </tr>
